...
def extract_vehicle_info(vehicle_contents, tag):
	#print(f"tage = {tag}")
	#print(f"vehicle = contents = {vehicle_contents}")
	# tag = "Team=.*\""
	#(Team=[^"]+")$
	new_tag = tag.split(".")[0]
	new_tag = "(" + new_tag + "\"[^\"]+\")$"
...
def get_vehicles(vehicle_dir, temp_dir, output_dir, vehicles, mod_mgr_path):
	...
	for vehicle in vehicles:
		...
		for idx, veh_dict in enumerate(veh_files_dicts.values()):
			...
			copy_dir = Path(output_dir, vehicle, veh_files[idx])
			create_dir(copy_dir)
			copy(Path(temp_dir, veh), copy_dir)
			copy(Path(temp_dir, veh_dict["dds_file"]), copy_dir)
			copy(Path(temp_dir, veh_dict["dds_region_file"]), copy_dir)
			copy(Path(temp_dir, veh_dict["json_file"]), copy_dir)
